...
function AdminhomePage(){
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        const fetchData = async () => {
          const formData = { 'W_ID': localStorage.getItem('Warehouse_ID')}
          const queryString = new URLSearchParams(formData);
          const url = `/api/WAnalytics?${queryString}`;
      ...
      useEffect(() => {
        const fetchData = async () => {
          const formData = { 'W_ID': localStorage.getItem('Warehouse_ID')}
          const queryString = new URLSearchParams(formData);
          const url = `/api/ProductWView?${queryString}`;
          const response = await fetch(url);
          const fetchedData = await response.json();
    ...
    const Restock = async () => {
        const W_ID = localStorage.getItem('Warehouse_ID')
        const response = await fetch('/api/RestockW', {
          method: 'POST',
  ...
  function AddProducts(){
    ...
    <div>
        ...
        <h1 style = {{textAlign:'center',marginBottom:'30px'}}>Manager: Warehouse-{localStorage.getItem('Warehouse_ID')}</h1><br /><br /><br />
        ...
        <button onClick={Restock}>Restock Items in Your Warehouse</button>
       
        <button onClick={viewInventory}>View Inventory</button>
        
        <button onClick = {Modification}>Modify Inventory</button><br />
